 ...
 *
 * Defence-in-depth that sits below any approval system: even if a command is
 * auto-approved, these checks block catastrophic operations like deleting system
 * files, exfiltrating credentials, or running fork bombs.
 *
 * Design principles:
...
export const CREDENTIAL_PATTERNS: LabelledPattern[] = [
  ...
  // Browser password databases — matches macOS/Windows Edge/Brave + Linux Chrome variants
  { pattern: /\/(?:(?:Google|google)[-_]Chrome|Microsoft\\\/Edge|BraveSoftware\/Brave-Browser|Microsoft\/Edge)\/.*\/Login Data$/i, label: 'browser password database' },
  { pattern: /\/(?:(?:Google|google)[-_]Chrome|Microsoft\\\/Edge|BraveSoftware\/Brave-Browser|Microsoft\/Edge)\/.*\/Cookies$/i, label: 'browser cookie database' },
  // Firefox
...
export const SYSTEM_DIR_PATTERNS_WIN32: LabelledPattern[] = [
  ...
  // 8.3 short names for Program Files
...
export const DANGEROUS_COMMAND_PATTERNS: DangerousPattern[] = [
  ...
  { pattern: /\bRemove-Item\b(?=[^;|&]*-Recurse)(?=[^;|&]*[\s'"][A-Z]:[\\\/](?=[^a-zA-Z0-9]|$))/im, reason: 'PowerShell recursive deletion of drive root.' },
  ...
  { pattern: /%0\s*\|\s*%0/, reason: 'Windows fork bomb detected.' },
  ...
  { pattern: /\breg\s+delete\s+HKEY_LOCAL_MACHINE\\/i, reason: 'System registry deletion.' },
...
export function validateFilePath(
  ...
  if (resolved.startsWith('\\\\')) {
    return { allowed: false, reason: 'Network (UNC) paths are blocked.' }
  }
  ...
  if (process.platform === 'win32' && (operation === 'write' || operation === 'delete')) {
    ...
    if (WINDOWS_DEVICE_NAME.test(basename)) {
      return {
        allowed: false,
        reason: `"${basename}" is a Windows reserved device name.`,
      }
    }
  }

  // Credential files (blocked for ALL operations)
